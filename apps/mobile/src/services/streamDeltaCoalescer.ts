...
/**
 * 合并窗口：120ms ≈ 8.3 次渲染/秒（视觉上仍是流畅的逐字出现）。
 *
 * 从 80ms 调大的原因：每次 flush 都会触发一次整条流式消息的 React 重渲染 + Fabric
 * commit（在移动端是主要成本）。把窗口从 80ms 拉到 120ms 可把流式期间的渲染次数
 * 再降约 1/3，直接减少卡顿来源，而 8.3 次/秒对"逐字出现"的观感几乎无差别。
 */
export const STREAM_DELTA_FLUSH_MS = 120
